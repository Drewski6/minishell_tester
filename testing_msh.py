...
# cwd = "/mnt/nfs/homes/nserve/Documents/42Cursus/minishell/minishell_00"
...
224,231,232,236,244,245,246,247,263,266,267,268,271,272,273,274,275,276,277,278,283,286,287,292,298,316,318,319,331,332,384, #env to the ignore list, only this one cd test is failing with -ne
 ...
 601,
 # 64,65,66,280,281,602,603,604,613,614,615,616,617,618,628, #tests bonus feature (parens)
...
745, 746, 747, 748, 749, 750, 751, 752, 753, 754, 755, 756, 757, 758, 759, 760, 761,        # super heredoc
]]
...
def run_test(test: str):
    ...
    returndiff = False
    if (args.non_interactive):
        input = f"{minishell_path} {test}",
    else:
        input = f"echo '{test}' | {minishell_path}",
    result_msh = sp.run(
        f"{input}",
        shell=True,
        stdout=sp.PIPE,
...
parser.add_argument(
    ...
    "-ni",
    action="store_true",
    help="test will run tests in non-interactive mode."
)
parser.add_argument(
    "--valgrind",
    "-vg",
